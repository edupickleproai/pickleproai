...
export default function SignupPage() {
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!email || !password) {
      setError('Please enter your email and choose a password.')
      return
    }
    setError('')
    ...
    <main className="min-h-screen bg-[#06080D] text-white">
      <div className="section-container flex min-h-screen items-center justify-center py-20">
        <div className="w-full max-w-lg rounded-[2rem] border border-white/10 bg-[#0D1524]/[0.96] p-8 shadow-[0_40px_120px_-50px_rgba(24,166,100,0.45)] backdrop-blur-sm">
          <div className="mb-8">
            <p className="text-sm uppercase tracking-[0.35em] text-slate-400">Sign Up</p>
            <h1 className="mt-4 text-4xl font-bold text-white">Create your AI pickleball profile</h1>
            <p className="mt-3 text-slate-400">Start your personalized onboarding and get the most from your coaching dashboard.</p>
          </div>
          ...
          <div className="mt-8 border-t border-white/10 pt-6 text-sm text-slate-400">
            <p>
              ...
              <Link href="/login" className="text-white hover:text-[#57FF00] transition">
                Log in
              </Link>
            </p>
          </div>
